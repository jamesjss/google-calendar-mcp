...
import { type CalendarEvent, type CalendarRef, type CreateEventInput, type UpdateEventInput, eventFingerprint, eventTimeWindow, toCandidate } from "../domain/calendar.js";
import { AppError } from "../domain/errors.js";

export class CalendarService {
  constructor(private readonly gateway: CalendarGateway) {}
  listCalendars() { return this.gateway.listCalendars(); }
  async resolveCalendar(ref?: CalendarRef): Promise<string> {
    ...
    if (matches.length > 1) throw new AppError(`Calendar name is ambiguous: ${ref.name}. Use its id.`, "AMBIGUOUS_CALENDAR", 409, matches);
    return matches[0]!.id;
  }
  async listEvents(ref: CalendarRef, query: ListEventsQuery) { return this.gateway.listEvents(await this.resolveCalendar(ref), query); }
  async getEvent(ref: CalendarRef, eventId: string) { return this.gateway.getEvent(await this.resolveCalendar(ref), eventId); }
  async createEvent(input: CreateEventInput): Promise<CalendarEvent> {
    ...
    if (input.duplicatePolicy !== "allow") {
      ...
      if (duplicate) throw new AppError("A matching event already exists; pass duplicatePolicy=allow only if intentional", "DUPLICATE_EVENT", 409, duplicate);
    }
  ...
  async updateEvent(input: UpdateEventInput): Promise<CalendarEvent> {
    ...
    for (const key of ["summary", "description", "location", "start", "end"] as const) if (input[key] !== undefined) Object.assign(patch, { [key]: input[key] });
    return this.gateway.updateEvent(calendarId, input.eventId, patch, input.etag);
  }
  async deleteEvent(ref: CalendarRef, eventId: string, etag?: string): Promise<void> { return this.gateway.deleteEvent(await this.resolveCalendar(ref), eventId, etag); }
}
