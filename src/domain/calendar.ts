...
export const UpdateEventSchema = z.object({
  ...
  if (event.start && event.end) {
    ...
    if (!bothDates && !bothDateTimes) {
      ctx.addIssue({ code: "custom", path: ["end"], message: "start and end must use the same representation" });
    } else if (bothDates && "date" in event.start && "date" in event.end && event.end.date <= event.start.date) {
      ctx.addIssue({ code: "custom", path: ["end", "date"], message: "end.date is exclusive and must be after start.date" });
    } else if (bothDateTimes && "dateTime" in event.start && "dateTime" in event.end) {
      validateTimedRange({ start: event.start, end: event.end }, ctx);
    }
  }
});

function validateTimedRange(
  ...
  if (end.toMillis() <= start.toMillis()) {
    ctx.addIssue({ code: "custom", path: ["end", "dateTime"], message: "end.dateTime must be after start.dateTime" });
  }
  for (const [key, value] of [["start", start], ["end", end]] as const) {
    const madridOffset = value.setZone(DEFAULT_TIME_ZONE).offset;
    if (value.offset !== madridOffset) {
      ctx.addIssue({
        code: "custom",
        path: [key, "dateTime"],
        message: `UTC offset does not match ${DEFAULT_TIME_ZONE} at that instant`,
      });
    }
  }
}
...
export function eventFingerprint(event: Pick<CalendarEvent, "summary" | "start" | "end" | "location">): string {
  ...
  return createHash("sha256").update(JSON.stringify({
    summary: normalize(event.summary),
    start: event.start,
    end: event.end,
...
export function eventTimeWindow(event: Pick<CalendarEvent, "start" | "end">): { timeMin: string; timeMax: string } {
  ...
  if ("dateTime" in event.start && "dateTime" in event.end) {
    return { timeMin: event.start.dateTime, timeMax: event.end.dateTime };
  }
